...
export function StatCard({ label, value, delta, deltaLabel, href }: {
  ...
  return (
    <Wrapper href={href} className={`bg-white border border-[#E5E7EB] rounded-lg px-4 py-3 flex flex-col gap-2 shadow-sm ${href ? 'hover:border-[#CBD5E1] hover:shadow transition-all' : ''}`}>
      <p className="text-[11px] font-semibold tracking-widest uppercase text-[#94A3B8] leading-4">{label}</p>
      <div>
