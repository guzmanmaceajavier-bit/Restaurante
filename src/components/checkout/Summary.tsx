...
interface IProps { orderData?: OrderData }

export function Summary({ orderData }: IProps) {
  const cart = useCartStore((state) => state.cart)

  return (
    <div className="bg-white rounded-2xl shadow-card border border-cream-200 p-6 md:sticky md:top-24 h-fit">
      <h2 className="text-xl font-display font-bold text-espresso-800 mb-4">Resumen del pedido</h2>
      {!cart.length ? (
        <p className="text-steel text-center py-8">No hay productos</p>
      ) : (
        <div className="space-y-3 mb-4">
          {cart.map((item) => (
            <div key={item.nombre} className="flex items-center gap-3 pb-3 border-b border-dashed border-cream-200 last:border-0">
              <img src={item.imagen} alt={item.nombre} className="w-12 h-12 rounded-xl object-cover" />
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm text-espresso-800 truncate">{item.nombre}</p>
                <p className="text-xs text-steel">x{item.quantity}</p>
              </div>
              <p className="font-semibold text-sm text-olive-500">${numberFormatter((item.precio ?? 0) * item.quantity)}</p>
            </div>
          ))}
        </div>
      )}
